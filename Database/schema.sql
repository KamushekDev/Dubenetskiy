...
--
-- Name: addprocess(character varying, bigint); Type: FUNCTION; Schema: public; Owner: postgres
--

CREATE FUNCTION public.addprocess(processname character varying, startstepid bigint, OUT process_id integer) RETURNS integer
    LANGUAGE plpgsql
    AS $$
BEGIN
...
ALTER FUNCTION public.addprocess(processname character varying, startstepid bigint, OUT process_id integer) OWNER TO postgres;

--
-- Name: addprocess(character varying, bigint, bigint); Type: FUNCTION; Schema: public; Owner: postgres
--

CREATE FUNCTION public.addprocess(processname character varying, startstepid bigint, roletostartid bigint, OUT process_id integer) RETURNS integer
    LANGUAGE plpgsql
    AS $$
BEGIN
    INSERT INTO runnable_processes (name, start_step_id, role_to_start_id)
    values (processName, startStepId, roleToStartId)
    RETURNING id INTO process_id;
END
$$;


ALTER FUNCTION public.addprocess(processname character varying, startstepid bigint, roletostartid bigint, OUT process_id integer) OWNER TO postgres;

--
-- Name: addprocesspermission(bigint, bigint); Type: FUNCTION; Schema: public; Owner: postgres
--

CREATE FUNCTION public.addprocesspermission(processid bigint, roleid bigint, OUT permission_id integer) RETURNS integer
    LANGUAGE plpgsql
    AS $$
BEGIN
...
ALTER FUNCTION public.addprocesspermission(processid bigint, roleid bigint, OUT permission_id integer) OWNER TO postgres;
...
CREATE FUNCTION public.linksteps(currentstepid bigint, nextstepid bigint, text character varying, roleids bigint[], OUT step_resolution_id integer) RETURNS integer
    LANGUAGE plpgsql
    AS $$
declare
    existing bigint;
